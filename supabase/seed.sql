...
select * from (values
  ...
  ('Abuja Prep', 'Maitama', 'Abuja', 'FCT', 'Sani Bello', 'admin@abujaprep.example', 280, 'interest', 'pending', 'waitlist')
...
from (values
  ...
  ('Corona School', 'David Musa', 'Year 4', 'Ikoyi', 'Motion sickness watch. Seat forward.', 'TRZ-WB-0004')
...
from routes
join vehicles on vehicles.registration_number = case when routes.type = 'morning' then 'TRZ-E012' else 'TRZ-D018' end
...
where routes.name in ('Lekki Morning Loop', 'Lekki Afternoon Loop')
  and not exists (
