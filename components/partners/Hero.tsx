...
import { BusFront, Factory, Fuel, ShieldCheck } from 'lucide-react'

const words = ['Made In Nigeria Fleet.', 'School Routes.', 'Safer Cities.']
const vehicles = Array.from({ length: 18 }, (_, i) => ({ top: 8 + (i % 6) * 14, delay: i * -1.4, duration: 18 + (i % 5) * 4, Icon: i % 3 === 0 ? BusFront : i % 3 === 1 ? Factory : Fuel }))
    ...
    <section className="relative min-h-screen overflow-hidden px-4 pt-32 pb-24 text-center" style={{ background: 'linear-gradient(120deg, #FFF0E4 0%, #FFF9F2 48%, #F1F6EA 100%)' }}>
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-8rem)] max-w-5xl flex-col items-center justify-center">
        ...
        <motion.p className="mx-auto mt-6 max-w-2xl text-base sm:text-lg leading-relaxed" style={{ color: '#65785F' }} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}>
          Tranzita partners with Nigerian vehicle assemblers, approved fleet suppliers, maintenance teams, depot operators, and fleet infrastructure providers. We are not accepting privately owned outside vehicles.
        </motion.p>
        <motion.div className="mt-8 flex flex-wrap justify-center gap-3" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }}>
          {['Vehicle Assemblers', 'Maintenance Partners', 'Fleet Infrastructure'].map((item) => <a key={item} href="#tiers" className="rounded-full px-5 py-3 text-sm font-extrabold" style={{ color: '#183024', background: 'rgba(255,249,242,0.82)', border: '1px solid #DDE9D2' }}>{item}</a>)}
        </motion.div>
