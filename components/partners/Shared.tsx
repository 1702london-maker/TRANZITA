...
export function SectionIntro({ label, title, text }: { label: string; title: string; text: string }) {
  return (
    <motion.div className="mx-auto max-w-4xl text-center" initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }} variants={fadeUp}>
      ...
      <p className="mx-auto mt-4 max-w-2xl text-base leading-relaxed" style={{ color: '#65785F' }}>{text}</p>
    </motion.div>
  )
}
