...
export default function DemoForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage(data.emailStatus === 'sent' ? 'Demo request received. The Tranzita team has been notified and will confirm your walkthrough.' : 'Demo request received. The Tranzita team will review it and contact you.')
    ...
    <section id="demo-form" className="py-24 px-4" style={{ background: '#F1F6EA' }}>
      <div className="max-w-6xl mx-auto grid lg:grid-cols-[0.85fr_1.15fr] gap-12 items-start">
        ...
        <motion.form onSubmit={submit} className="gradient-frame rounded-3xl p-6 sm:p-8 bg-white space-y-5" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          {message && (
            <div role="status" className="rounded-2xl p-4 text-center" style={{ background: '#FFF0E4', border: '1px solid rgba(217,107,31,0.22)' }}>
              <p className="text-sm font-extrabold" style={{ color: '#183024' }}>{message}</p>
              <p className="mt-1 text-xs font-bold" style={{ color: '#65785F' }}>We normally respond within 4 business hours.</p>
            </div>
          )}
