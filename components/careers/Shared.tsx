...
export function IconCards({ cards, columns = 'md:grid-cols-3' }: { cards: Array<[string, LucideIcon, string]>; columns?: string }) {
  return (
    <div className={`grid ${columns} gap-6`}>
      {cards.map(([title, Icon, body], i) => (
        <motion.div key={title} className="gradient-frame rounded-2xl p-6" style={{ background: '#FFFFFF' }} initial={{ opacity: 0, y: 34 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -5 }}>
